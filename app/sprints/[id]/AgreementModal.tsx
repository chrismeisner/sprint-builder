...
export default function AgreementModal({ agreement, onClose, onRegenerate, isRegenerating }: Props) {
  ...
  const renderMarkdown = (md: string) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmedLine.startsWith("# ")) {
        elements.push(
          <h1 key={i} className={`${getTypographyClassName("h2")} mt-6 mb-4 first:mt-0`}>
            {trimmedLine.slice(2)}
          </h1>
        );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-white dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-black/10 dark:border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            <h2 className={getTypographyClassName("h3")}>Generated Agreement</h2>
        ...
        <div className="flex items-center gap-2 px-4 py-2 border-b border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02] shrink-0">
          <button
            onClick={() => setShowRaw(false)}
            className={`${getTypographyClassName("button-sm")} px-3 py-1.5 rounded-md transition ${
              !showRaw 
          ...
          <p className={`${getTypographyClassName("body-sm")} text-text-muted`}>
            Copy and paste into Google Docs for eSignature
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {showRaw ? (
            <pre className="whitespace-pre-wrap font-mono text-sm bg-black/[0.02] dark:bg-white/[0.02] p-4 rounded-lg border border-black/10 dark:border-white/10">
              {agreement}
            </pre>
          ) : (
        ...
        {/* Footer */}
        <div className="flex items-center justify-between gap-3 p-4 border-t border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02] shrink-0">
          <div className="flex items-center gap-2">
            {onRegenerate && (
              <button
                onClick={onRegenerate}
                disabled={isRegenerating}
                className={`${getTypographyClassName("button-sm")} px-3 py-2 rounded-md border border-black/10 dark:border-white/15 hover:bg-black/5 dark:hover:bg-white/10 disabled:opacity-50 transition flex items-center gap-2`}
              >
                {isRegenerating ? (
                  <>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleCopy}
              className={`${getTypographyClassName("button-sm")} px-4 py-2 rounded-md bg-black dark:bg-white text-white dark:text-black hover:opacity-90 transition flex items-center gap-2`}
            >
              {copied ? (
                <>
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
