  ...
  return (
    <div className="flex items-center gap-2">
      <select
        ...
        disabled={isChanging}
        className={`px-2.5 py-1 rounded border border-white/20 bg-white/10 text-white/90 ${getTypographyClassName("body-sm")} disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-1 focus:ring-white/30`}
      >
      ...
      {isChanging && (
        <svg className="animate-spin h-3.5 w-3.5 text-white/60" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
        </svg>
      )}
      {success && (
        <span className={`${getTypographyClassName("body-sm")} text-green-400`}>✓</span>
      )}
      {error && (
        <span className={`${getTypographyClassName("body-sm")} text-red-400`} title={error}>✗</span>
      )}
    </div>
  );
}
