...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    await pool.query(
      `UPDATE sprint_drafts SET contract_pdf_url = $1, updated_at = now() WHERE id = $2`,
      [url, params.id]
    );

    return NextResponse.json({ url }, { status: 201 });
  } catch (error: unknown) {
    console.error("[AgreementPDF Upload] Error:", error);
    return NextResponse.json(
...
export async function DELETE(request: Request, { params }: Params) {
  try {
    ...
    await pool.query(
      `UPDATE sprint_drafts SET contract_pdf_url = NULL, updated_at = now() WHERE id = $1`,
      [params.id]
    );

    return NextResponse.json({ success: true });
  } catch (error: unknown) {
    console.error("[AgreementPDF Delete] Error:", error);
    return NextResponse.json(
