...
import { normalizeTimeRange } from "@/app/sandboxes/miles-proto-5/_lib/format-time";
...
export function TripListItem({
  ...
  return (
    ...
    >
      ...
      {/* 2-col grid keeps locations aligned regardless of time width — column 1
          auto-sizes to whichever time string is longest within this row. */}
      <div className="grid min-w-0 flex-1 grid-cols-[auto_minmax(0,1fr)] items-center gap-x-3 gap-y-1">
        <span className="text-left text-xs leading-none tabular-nums text-text-muted">
          {startTime}
        </span>
        <span className="block min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-sm font-medium leading-snug text-text-primary">
          {trip.from}
        </span>
        <span className="text-left text-xs leading-none tabular-nums text-text-muted">
          {endTime ?? ""}
