...
import { BASE, p } from "@/app/sandboxes/miles-proto-5/_lib/nav";
import { AskMilesBadge } from "@/app/sandboxes/miles-proto-5/_components/ask-miles-badge";

/**
 * Global iOS-style top navigation bar.
 *
 *  • Mounted once in layout.tsx — every page gets it for free.
 *  • Sticky, safe-area aware, sits above scroll content like a UINavigationBar.
 *  • Suppressed on chrome-free flows (welcome, signup, onboarding utilities,
 *    full-bleed surfaces). Mirrors bottom-nav's HIDDEN_ON set.
 *
 * Iterate from here:
 *   - per-page overrides via context provider (custom title, back chevron,
 *     trailing actions) when first concrete need arrives.
 *   - large-title variant (iOS Large Title pattern) when scrolled to top.
 */
...
const HIDDEN_ON = new Set([
  ...
  ...[
    ...
    "/help-port",
    "/help-port/vehicle",
    "/help-port/vehicle/result",
    "/help-port/vin",
    "/help-port/vin/result",
    "/device-detected",
    "/whos-driving",
    "/index",
    "/home-screen",
...
function MilesWordmark() {
  return (
    <svg
      viewBox="80 190 690 365"
      className="h-[30px] w-auto text-neutral-900 dark:text-neutral-100"
...
export function AppHeader() {
  ...
  // own their full top of screen, with the safe-area-inset handled by
  // an invisible spacer below so content clears the notch / status bar.
  const isDashboard =
  ...
  return (
    ...
    >
      <Link
        ...
        className="flex items-center rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
      >
        <MilesWordmark />
      </Link>
