...
const TABS: TabDef[] = [
  ...
  },
  {
    ...
    label: "Trips",
    icon: TripsIcon,
    // Children of the Trips flow (trip-detail, event-detail, driver-reassignment)
    // keep the Trips tab highlighted so users don't lose their place.
    match: (pn) =>
      pn.startsWith(`${BASE}/trips`) ||
      pn.startsWith(`${BASE}/trip-`) ||
      pn.startsWith(`${BASE}/event-detail`) ||
      pn.startsWith(`${BASE}/driver-reassignment`),
  },
  {
    kind: "sheet",
    id: "miles",
...
function BottomNavInner() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("miles-proto-5-footer-nav-mode-change", handleCustomChange);
    return () => {
      window.removeEventListener("storage", handleStorage);
      window.removeEventListener("miles-proto-5-footer-nav-mode-change", handleCustomChange);
    };
  }, []);
