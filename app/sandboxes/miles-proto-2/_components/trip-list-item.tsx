...
interface TripListItemProps {
  ...
  href: string;
  className?: string;
  /** Driver photo URL — replaces the driver initial badge */
  driverImageSrc?: string;
  /** Vehicle photo URL — replaces the car SVG icon */
...
export function TripListItem({
  ...
  return (
    ...
    >
      <div className="relative flex size-9 shrink-0 items-center justify-center rounded-full bg-neutral-100">
        {vehicleImageSrc ? (
          <img src={vehicleImageSrc} alt="vehicle" className="size-9 rounded-full object-cover" />
        ) : (
        ...
        {driverImageSrc ? (
          <img
            src={driverImageSrc}
            alt={trip.driverInitials ?? "driver"}
            className="absolute -bottom-0.5 -right-0.5 size-5 rounded-full border-2 border-white object-cover"
          />
        ) : trip.driverInitials ? (
          <span
            className="absolute -bottom-0.5 -right-0.5 flex size-4 items-center justify-center rounded-full border border-white bg-neutral-700 text-[8px] font-semibold leading-none text-white"
