...
const stackSections: Array<{
  ...
  },
  {
    ...
    items: [
      ...
      {
        name: "Typeform Intake Webhooks",
        summary: "Incoming project requests post to `/api/documents` where signatures are verified with `lib/typeform.ts` before persisting.",
        files: ["app/api/documents/route.ts", "lib/typeform.ts"],
  ...
  {
    title: "AI & Automation",
    description: "Services that automate sprint generation from intake docs.",
    items: [
      {
        name: "OpenAI (gpt-4o / 4o-mini)",
        summary: "Sprint drafts are generated by hitting the Chat Completions API with catalog context in `lib/sprint-creation.ts`.",
        files: ["lib/sprint-creation.ts", "app/ai-test/*", "app/api/documents/[id]/sprint/route.ts"],
        env: ["OPENAI_API_KEY", "OPENAI_PROJECT_ID (optional)", "OPENAI_ORG_ID (optional)"],
        statusKey: "openai",
        docs: "https://platform.openai.com/docs",
      },
      {
        name: "Prompt + Messaging Store",
        summary: "System prompts and brand language blocks live in `lib/prompts.ts` and `lib/messaging.ts` so AI output stays on-brand.",
        files: ["lib/prompts.ts", "lib/messaging.ts"],
      },
    ],
  },
  {
